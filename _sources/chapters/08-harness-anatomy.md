...
第 07 章论证了：任何一具能工作的马具，都必须按那个因果顺序扛住三位承重的护法——SDD、TDD、MDD。本章把这三位护法放到四条操作性的 **区域（zones）**——*缰绳（Bridle）*、*护栏（Fence）*、*牧场（Paddock）*、*梳理（Groom）*——上，得到一张 3 行 × 4 列的矩阵。这十二格，就是本书后续每一章都会回头指的那条分析脊梁。
...
```{literalinclude} ../_handson/08-harness-anatomy/sdd-x-bridle/AGENTS.md.sample
:language: markdown
```
...
```{literalinclude} ../_handson/08-harness-anatomy/sdd-x-fence/prompt-schema.json
:language: json
```
...
```{literalinclude} ../_handson/08-harness-anatomy/sdd-x-paddock/acceptance-gate.md
:language: markdown
```
...
```{literalinclude} ../_handson/08-harness-anatomy/sdd-x-groom/update-docs.sh
:language: bash
```
...
```{literalinclude} ../_handson/08-harness-anatomy/tdd-x-bridle/test_loop.py
:language: python
```
...
```{literalinclude} ../_handson/08-harness-anatomy/tdd-x-fence/hooks.json
:language: json
```
...
```{literalinclude} ../_handson/08-harness-anatomy/tdd-x-paddock/ci-gate.yml
:language: yaml
```
...
```{literalinclude} ../_handson/08-harness-anatomy/tdd-x-groom/flaky-test-quarantine.md
:language: markdown
```
...
- **那条从来没被写出来的敌意测试。** 那条人本 *应该* 写但没写的测试，正是能找出智能体捷径的那条。若 PR 里从不追加"再写一条专门攻击通向绿的最省力路径的测试"（参见第 07 章"第一把就过"那条陷阱），TDD × 牧场就会慢慢地 *训练* 智能体，而不是防它。
...
```{literalinclude} ../_handson/08-harness-anatomy/mdd-x-bridle/metrics-north-star.md
:language: markdown
```
...
```{literalinclude} ../_handson/08-harness-anatomy/mdd-x-fence/cost-cap.yaml
:language: yaml
```
...
```{literalinclude} ../_handson/08-harness-anatomy/mdd-x-paddock/release-sli.md
:language: markdown
```
...
```{literalinclude} ../_handson/08-harness-anatomy/mdd-x-groom/weekly-audit.sh
:language: bash
```
...
`source/_handson/08-harness-anatomy/` 下住着十二份制品，每格一份，按 `<guardian>-x-<zone>/<filename>` 的目录结构排布：
...
每一份制品的内容都已内联渲染在上面各自的 H3 小节里；磁盘上的那份文件是唯一真源。想一格一格上线的读者，可以拷出任一份文件开始动手。本书在第 16 章的核心建议是：一支团队应当在 **60 天内上线完整的一行或完整的一列**——也就是三十天读本章，另外三十天交付四份制品。
