...
第 03 章说，Harness Engineering 的主语不是模型，而是模型周围的结构。第 03B 章会拆 Agent Loop。本插章夹在二者之间，只讲其中一块最容易被低估、也最容易腐烂的结构：**记忆系统**。
...
## 03A.2 五层记忆：temp、short、long、archive、meta

一个实用的 Agent Memory 系统，至少要把物理存储拆成五层。下面这份分层来自一个真实 LazyBot 记忆增强设计：从 Markdown append-only 文件升级为 SQLite-backed store，同时保留关键词召回，并在 sqlite-vec 可用时启用语义召回。
...
`agents-md-generate` 这个 skill 里有一条非常重要的隐含原则：**面向智能体的长期规则，应当先进入可审阅、可 diff、可链接的制品，再被记忆系统索引或召回。** `AGENTS.md` 是仓库级入口；`SKILL.md` 是流程级记忆；ADR 和 PKB 是架构级记忆。向量库、SQLite 表、摘要缓存都可以加速召回，但它们不应该成为唯一真源。
...
## 03A.6 存储形态：一个 active DB，加历史 rolled DB
...
第 05 章会把"梳理（Groom）"列正式放进马具矩阵。记忆系统是最能说明这列必要性的例子：记忆的价值会随时间衰减，而它造成的行为影响却会持续存在。
...
本章的两份 hands-on 制品住在 `source/_handson/03a-agent-memory-anatomy/`：

- `memory-layer-map.yaml` —— 一份五层 memory map，可直接拷进项目做设计检查。
- `memory-groom-checklist.md` —— 一份梳理检查表，专门检查规则、最佳实践、常见陷阱是否仍然可信。

```{literalinclude} ../_handson/03a-agent-memory-anatomy/memory-layer-map.yaml
:language: yaml
```

```{literalinclude} ../_handson/03a-agent-memory-anatomy/memory-groom-checklist.md
:language: markdown
```
