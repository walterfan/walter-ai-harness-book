...
OpenHarness 由香港大学数据科学实验室（HKUDS）编写，以 MIT 许可开源发布 {cite}`hkuds2025openharness`。它是本书最常回头引用的那份参考实现。读它的感觉，就像作者们已经先写过第 08 章：仓库整齐地分成十个子系统，每一个都刚好对应到"三护法 × 四区域"矩阵里的某一格、或某一小簇格子。本章逐个走过这些子系统、引用 Agent Loop 核心那一小段代码，并给整个项目按第 08 章的矩阵打一次分。
...
6. `engine/` —— Agent Loop 本身（见下文 10.2）。
...
## 10.2 —— Agent Loop（不超过 20 行，MIT 署名）
...
本书的每一章案例研究，都会按第 08 章的十二格、以 0–5 分制给马具打分，并用 1–3 句话、引用 `oss/OpenHarness/` 下的具体文件作为证据。
...
- **梳理被留给了运维。** 10.4 里梳理列 2.3 的均值是诚实的信号：这个项目交付的是一台引擎和一台沙箱，而不是一套维护纪律。若一支团队把 OpenHarness 整套搬进来，不加上自己的每周审计、自己的 doc-sync 检查、自己的 HarnessCard 节奏，那这具马具的衰减速度会和任何一份没人照料的仓库一样——第 09 章的那份熵关切，*不会* 由这个框架替你提供。
- **没有宣布过一条北极星度量。** 10.4 给 MDD × 缰绳 的打分是 3（*"事件流存在，但没有一条被正式宣布的北极星"*）。第 07 章的警告在这里适用：一具没有人持有、没有阈值看护的北极星的马具，会收集一堆 dashboard，却哪一块都引导不了。OpenHarness 给的是原料，宣布度量这件事是你的职责。
...
在 `source/_handson/10-openharness/` 下，住着两份可直接拷走的制品：
