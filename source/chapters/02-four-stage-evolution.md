...
上面这套四阶段的读法，底下托着五束研究脉络；第 4 章在给出"三大护法"这个命名时，会再次捡起同样的主题。
...
四份阶段文件加起来的总制品预算：≤ 40 行。这个约束本身就是点题：**马具不比技能长，技能也不比上下文长**——每一个阶段加的是 *杠杆*，不是体量。你在阶段 4 省下来的那些行，正是你在第 5 章搭建"三大护法 × 四区域"矩阵时要花出去的那些行。
