...
**术语约定**：讲稿里涉及 `CLAUDE.md / AGENTS.md / SKILL.md /
...
模型周围的环境**；我们部门写代码的方式正在被这一转移重塑，而且
再不讨论一次，下一轮 code review 上我们就会各说各话。
...
**回书链接.** 《Foreword》（第 01 章）及《The Four-Stage Evolution》（第 02 章）。
...
**回书链接.** 第 02 章《The Four-Stage Evolution》完整论证，尤其是
...
1. **面向智能体的规约** —— `CLAUDE.md` ／ `AGENTS.md` ／ `SKILL.md` ／
...
**回书链接.** 第 03 章《What is Harness Engineering?》，尤其是 §03.1
...
——`CLAUDE.md` ／ `AGENTS.md` ／ skill 文件**。如果规约含糊，
...
**回书链接.** 第 04 章《The Three Guardians》，特别是「为什么因果
顺序要翻转」那一节。
...
**回书链接.** 第 05 章《Harness Anatomy》§05.Overview 和 §05.Provenance。
...
- **OpenHarness（第 07 章，港大 DS Lab，开源）** —— *唯一一份
  "就像读过第 05 章之后才写的" 开源参考实现*。十个子系统整齐对
  应到矩阵格子；Runtime 层 Paddock 尤其强。警醒：它是参考实现，
  不是你的运行时。
- **Superpowers（第 08 章，Joseph Vincent，开源）** —— *OpenHarness
  的哲学反面。只出技能库，不出运行时*。三十几份 `SKILL.md`，
  强在 SDD × Bridle。警醒："光靠技能能不能做我们的挽具？" ——
  *不能*，少了 Fence 与 Paddock。
- **lazy-scrum-team（第 09 章，开源 skill 包）** —— *把 Scrum 角色
  与交接本身当挽具*。三件可复用的模式：Artefact State Model、
  Rework Matrix、Hard vs Soft Gates。
- **Claude Code（第 10 章，Anthropic，闭源；经由张汉东《马书》分析）** ——
  *本书唯一的闭源案例*。挽具职责分布在 bundled prompt ＋ hooks 契约
  ＋ skills 系统 *三处*。警醒：闭源意味着你看得到行为但拿不到源，
  分析结论永远带一个有效期标注。

**一句带走.** *矩阵是尺子，案例是用它量出来的刻度；要你部门抄
作业，选一个最接近你们技术栈的案例。*
...
改不动，那它就是错的。* 本书第 11 章把 3 × 4 矩阵扣在
`walterfan/lazy-ai-coder` 自己头上，做了一场四幕实盘。
...
- **Act 2 · 短板.** 列出五个具体短板，每一个都 **打上严重度、
  ...
  - ……
- **Act 3 · 修复.** 五笔真实 commit 进 main；`reproduce.sh` 可复现。
- **Act 4 · 度量 delta.** 重新打分：
...
**回书链接.** 第 11 章《Lazy AI Coder Four-Act Worked Example》与
附录 D《HarnessCard Template》。
...
- **"Day 30 我们就搞定了"陷阱** —— 没人给挽具本身排维护班，90 天后
  那格的分数从 +1 掉回 +0.3。
...
**回书链接.** 第 12 章《Where We Go from Here》§12.2「30／60／90-Day
Action Checklist」与其配套的 pitfalls。
...
- *Q：这跟我们现有的 CI 流水线有什么区别？*
  - *A：CI 是 TDD ／ MDD 的 **牧场**。它在合入那一刻发声；挽具在
...
- 30/60/90 清单：`_handson/12-where-we-go-from-here/checklist-30-60-90.md`
- HarnessCard 模板：附录 D
- `CLAUDE.md` 样板：附录 E
- 精选阅读单：附录 C
